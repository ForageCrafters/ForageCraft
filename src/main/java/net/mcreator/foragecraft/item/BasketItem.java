...
import net.mcreator.foragecraft.item.inventory.BasketInventoryCapability;
...
public class BasketItem extends Item {
	public BasketItem() {
		super(new Item.Properties().tab(ForagecraftModTabs.TAB_FORAGE_CRAFT_TOOLS).stacksTo(1).rarity(Rarity.COMMON));
	}
		...
		if (entity instanceof ServerPlayer serverPlayer) {
			NetworkHooks.openScreen(serverPlayer, new MenuProvider() {
				...
				public Component getDisplayName() {
					return Component.literal("Wicker Basket");
				}
				...
				public AbstractContainerMenu createMenu(int id, Inventory inventory, Player player) {
					...
					return new WickerBasketGUIMenu(id, inventory, packetBuffer);
				}
			}, buf -> {
	...
	public ICapabilityProvider initCapabilities(ItemStack stack, @Nullable CompoundTag compound) {
		return new BasketInventoryCapability();
	}
