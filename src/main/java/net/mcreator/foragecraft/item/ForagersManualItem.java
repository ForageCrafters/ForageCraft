...
import net.mcreator.foragecraft.procedures.ForagersManualRightclickedProcedure;
import net.mcreator.foragecraft.item.renderer.ForagersManualItemRenderer;
...
public class ForagersManualItem extends Item implements IAnimatable {
	public AnimationFactory factory = GeckoLibUtil.createFactory(this);
	public String animationprocedure = "empty";
	public static ItemTransforms.TransformType transformType;

	public ForagersManualItem() {
	...
	public void initializeClient(Consumer<IClientItemExtensions> consumer) {
		...
		consumer.accept(new IClientItemExtensions() {
			private final BlockEntityWithoutLevelRenderer renderer = new ForagersManualItemRenderer();
	...
	protected <P extends IAnimatable> void customInstructionListener(CustomInstructionKeyframeEvent<P> event) {
		...
		if (!(ister instanceof ForagersManualItemRenderer))
			return;
		ForagersManualItemRenderer renderer = (ForagersManualItemRenderer) ister;
		instructionTokens.stream().filter(tks -> !tks.isEmpty()).forEach(tks -> this.interpretFirstPersonInstructions(tks, renderer));
	}

	protected void interpretFirstPersonInstructions(List<String> tokens, ForagersManualItemRenderer renderer) {
	...
	private <P extends Item & IAnimatable> PlayState idlePredicate(AnimationEvent<P> event) {
		if (this.transformType != null ? true : false) {
			if (this.animationprocedure.equals("empty")) {
				event.getController().setAnimation(new AnimationBuilder().addAnimation("ForagersManual.idle", EDefaultLoopTypes.LOOP));
	...
	public void setupAnimationState(ForagersManualItemRenderer renderer, ItemStack stack, PoseStack matrixStack, float aimProgress) {
	}
	...
	public InteractionResultHolder<ItemStack> use(Level world, Player entity, InteractionHand hand) {
		...
		ForagersManualRightclickedProcedure.execute(world, x, y, z, entity, itemstack);
		return ar;
	}
}
